...
class EditUnits():
    ...
    def __del_unit(self):
        ...
        nameUnitSelected = self.__myUnits.Lista[editar.option].Name
    ...
    def __choose_unit(self):
        ...
        editar = Menu(opciones, 'Elija la unidad a editar', True)
        if editar.option is None:
            return None
        else:
            return self.__myUnits.get_unit(opciones[editar.option]['item'])
    ...
    def __selectDev(self, USBOnly=True):
        ...
        editar = Menu(_optMenu, f"{len(_lista)} dispositivos conectados", True)

        if editar.option is None:
            return None
        else:
            return _lista[editar.option]
    ...
    def __selectPart(self, dev):
        ...
        editar = Menu(_optMenu, f"Particiones en {dev.path}", True)

        if editar.option is None:
            return None
        return dev.partitions[editar.option]
